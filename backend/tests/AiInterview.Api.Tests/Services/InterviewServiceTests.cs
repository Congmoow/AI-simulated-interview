...
public class InterviewServiceTests
{
    ...
    [Fact]
    public async Task CreateInterviewAsync_ShouldUseLocalOpeningQuestion_WhenAiStartIsUnavailable()
    {
        var userId = Guid.NewGuid();
        var position = new Position
        {
        ...
        var service = new InterviewService(
            ...
            catalogRepository,
            new InMemoryReportRepository(),
            new FailingAiIntegrationService(),
            new StubAiSettingsService(),
            new StubInterviewReportGenerationQueue(),
            new StubHubContext(),
            NullLogger<InterviewService>.Instance);
        ...
        result.PositionName.Should().Be(position.Name);
        result.FirstQuestion.QuestionId.Should().Be(question.Id);
        result.FirstQuestion.Type.Should().Be(question.Type);
        result.FirstQuestion.Title.Should().Be(question.Content);
        result.Messages.Should().ContainSingle();
        result.Messages.Single().Content.Should().Be(question.Content);
        interviewRepository.Interview.Should().NotBeNull();
    }
    ...
    [Fact]
    public async Task ProcessInterviewAsync_ShouldAcceptDirectProviderReportJson_WithoutFallback()
    {
        var userId = Guid.NewGuid();
        var interviewId = Guid.NewGuid();
        var interviewRepository = new InMemoryInterviewRepository
        ...
        var aiSettingsService = new StubAiSettingsService
        {
            Provider = new StubAiProvider
            {
                RawResponse =
                    """
                    {
                      "executiveSummary": "report summary from direct provider",
                      "strengths": ["clear structure"],
                      "weaknesses": ["depth can improve"],
                      "detailedAnalysis": {
                        "technicalAccuracy": "strong evidence"
                      },
                      "learningSuggestions": ["add more production metrics"],
                      "trainingPlan": [],
                      "nextInterviewFocus": ["database consistency"]
                    }
                    """
            }
        };
        ...
        aiIntegrationService.GenerateReportCallCount.Should().Be(0);
        reportRepository.SavedReport.Should().NotBeNull();
        reportRepository.SavedReport!.ExecutiveSummary.Should().Be("report summary from direct provider");
        reportRepository.SavedReport!.LearningSuggestions.Should().ContainSingle().Which.Should().Be("add more production metrics");
        reportRepository.SavedReport!.NextInterviewFocus.Should().ContainSingle().Which.Should().Be("database consistency");
        interviewRepository.Interview!.Status.Should().Be(InterviewStatuses.Completed);
        hubContext.HubClients.SharedClient.ReportReadyPayloads.Should().ContainSingle();
    }
