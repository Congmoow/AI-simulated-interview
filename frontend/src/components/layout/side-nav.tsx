    ...
    <aside className="sticky top-4 flex h-[calc(100vh-2rem)] w-full max-w-[288px] flex-col justify-between overflow-y-auto p-6">
      <div className="space-y-6">
        ...
        <nav className="space-y-3">
          {navItems.filter((item) => item.href !== "/admin" || user?.role?.toLowerCase() === "admin").map((item) => {
            const active =
              pathname === item.href ||
            ...
            {user ? (
              <>
                <div className="border-b border-[var(--token-color-border-default)] px-4 py-3">
                  <p className="text-[length:var(--token-font-size-xs)] text-[var(--token-color-text-tertiary)]">已登录账户</p>
                  <p className="mt-0.5 font-semibold text-[length:var(--token-font-size-sm)] text-[var(--token-color-text-primary)]">{user.username}</p>
                </div>
              ...
              <button
                className="w-full px-4 py-2.5 text-left text-[length:var(--token-font-size-sm)] text-[var(--token-color-text-primary)] transition-colors hover:bg-[rgba(17,24,39,0.04)]"
                onClick={() => { setMenuOpen(false); openLogin(null); }}
                type="button"
              >
                去登录
              </button>
