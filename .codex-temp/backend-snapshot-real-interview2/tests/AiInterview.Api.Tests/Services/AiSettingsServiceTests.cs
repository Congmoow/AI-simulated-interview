...
public class AiSettingsServiceTests
{
    ...
    public async Task UpdateSettingsAsync_WhenApiKeyIsBlank_ShouldKeepExistingProtectedKey()
    {
        ...
        repository.Current.ApiKeyMasked.Should().Be("mask::key");
        result.IsKeyConfigured.Should().BeTrue();
        result.ApiKeyMasked.Should().Be("mask::key");
    }
    ...
    {
        var service = new AiSettingsService(
            new InMemoryAiSettingsRepository(new AiProviderSetting
            {
                Provider = "openai_compatible",
                BaseUrl = "https://example.com/v1",
                Model = "gpt-test",
    ...
    [Fact]
    public async Task GetRuntimeSettingsAsync_WhenEnabledAndConfigured_ShouldReturnDecryptedSettings()
    {
        var service = new AiSettingsService(
            new InMemoryAiSettingsRepository(new AiProviderSetting
            {
                ...
                BaseUrl = "https://example.com/v1",
                Model = "qwen-plus",
                ApiKeyProtected = "protected::real-key",
                IsEnabled = true,
                Temperature = 0.4m,
                MaxTokens = 1024,
                SystemPrompt = "prompt",
        ...
        result.Should().NotBeNull();
        result!.Provider.Should().Be("qwen");
        result.BaseUrl.Should().Be("https://example.com/v1");
        result.Model.Should().Be("qwen-plus");
        result.ApiKey.Should().Be("real-key");
        result.SystemPrompt.Should().Be("prompt");
    }

    [Fact]
    public async Task GetRuntimeSettingsAsync_WhenDisabled_ShouldReturnNull()
    {
        var service = new AiSettingsService(
            new InMemoryAiSettingsRepository(new AiProviderSetting
            {
