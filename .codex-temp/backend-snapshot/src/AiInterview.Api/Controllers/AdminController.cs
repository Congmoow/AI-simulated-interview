...
[Route("api/v1/admin")]
public class AdminController(IAdminService adminService, IAiSettingsService aiSettingsService) : ApiControllerBase
{
    [HttpPost("questions")]
    ...
    {
        var result = await aiSettingsService.GetSettingsAsync(cancellationToken);
        return Success(result, "查询成功");
    }
    ...
    public async Task<IActionResult> UpdateAiSettings([FromBody] UpdateAiSettingsRequest request, CancellationToken cancellationToken)
    {
        ...
        var result = await aiSettingsService.UpdateSettingsAsync(request, updatedBy, cancellationToken);
        return Success(result, "AI 配置保存成功");
    }

    [HttpPost("ai-settings/test")]
    public async Task<IActionResult> TestAiConnection([FromBody] TestAiConnectionRequest? request, CancellationToken cancellationToken)
    {
        var result = await aiSettingsService.TestConnectionAsync(request, cancellationToken);
        return Success(result, result.Success ? "连接测试成功" : "连接测试失败");
    }
}
